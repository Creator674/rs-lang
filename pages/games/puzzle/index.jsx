...
const  Puzzle = (props) => {
  ...
  const checktheAnswer = (result) => {
    ...
    if(result[0].join('') === result[1].join('')){
      ...
      setMistakes(result[1].filter(( el, ind) => {
        if(el !== result[0][ind]){
          return el; 
        }
      }));
  ...
  return (
    <div className='puzzle-wrapper'>
      ...
      <GameboardPuzzle pronunc={pronunc} translat={translat}
                 showImage={showImage} autoPronunc={autoPronunc}
                 data={data[count]} dontKnow={dontKnow}
                 goOn={goOn} goToNext={goToNext} picture={picture}
                 mistakes={mistakes} />
     
      <FooterPuzzle checktheAnswer={checktheAnswer} clickOnNext={clickOnNext}
              goToNext={goToNext} result={result}
              clickIdontKnow={clickIdontKnow} readyToCheck={readyToCheck}/>
    </div>
  )
}
